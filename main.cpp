...
MatrixXd ITM(double x,double y,double th,MatrixXd veclist){
    Matrix3d itm ;itm << cos(th), -sin(th),x,
                        sin(th) ,cos(th),y,
                        0,0,1;
...
MatrixXd lidarlist(double x,double y, double th,MatrixXd lines0){
    ...
    for (int i=0;i<beemn;i++){
        ...
        xrobo=x;yrobo=y;
        xbeem=x+R*cos(th+thetas(i));ybeem=y+R*sin(th+thetas(i));
    ...
    MatrixXd LiDAR(2,beemn) ;LiDAR<< lidarxlist, lidarylist;
    return LiDAR;
}
...
for(int k=0;k<50;k++){
    ...
    for (int j=0; j<A.rows();j++){
        double x0,y0; x0=A(j,0);y0=A(j,1);
...
// cout<<Rm<<endl;
// cout<<endl;
if (Rm(0,1)*Rm(1,0)>0){Rm= MatrixXd::Identity(2,2);}
...
xc=Posi(0);yc=Posi(1);thc=Posi(2);
...
for (int t=0;t<N;t++){
    ...
    dxr=drr*cos(dthr);
    dyr=drr*sin(dthr);
    MatrixXd dvecr(2,1);dvecr<<dxr,dyr;
    MatrixXd davr=ITM(xr,yr,thr,dvecr);

    xr=(davr(0,0));yr=(davr(1,0));thr=(thr+dthr);
...
cout<<"robot"<<endl;
int rn= 7;
RowVectorXd robox(7);robox<<0,-1,-3,-3,-3,-1,0;
RowVectorXd roboy(7);roboy<<0,   1.,  1,0,-1  ,-1,0;
robox = 400*robox/3; roboy =400*roboy/2;
double avx=robox.mean(); double avy=roboy.mean();
robox -=avx*VectorXd::Ones(rn);
roboy -=avy*VectorXd::Ones(rn);
MatrixXd robovec(2,7) ;robovec<<robox,roboy;
////////////////////////////////////////////////////
return 0;
}
